...
impl QuietHistory {
    pub fn get(&self, threats: Bitboard, stm: Color, mv: Move, is_pawn: bool) -> i32 {
        let entry = &self.entries[stm][is_pawn as usize][mv.from()][mv.to()];
        (entry.factorizer + entry.bucket(threats, mv)) as i32
    }

    pub fn update(&mut self, threats: Bitboard, stm: Color, mv: Move, is_pawn: bool, bonus: i32) {
        let entry = &mut self.entries[stm][is_pawn as usize][mv.from()][mv.to()];
